...
const diskStorageFacturas = multer.diskStorage({
  destination: (req, file, cb) => { 
    cb(null, path.join(__dirname,"../../public/uploads/images/comprainsumos"));
  },
...
/**
 * @memberof config/files
 * @function uploadImagenFactura
 * @param {function} storage Recibe, crea y almacena la información del archivo en servidor
 * @param {number} limits Limite del tamaño máximo del archivo que permitirá el multer
 */
exports.uploadImagenFactura = multer({
  storage: diskStorageFacturas,
  fileFilter: (req, file, cb) => {
    if (file.mimetype == "image/png" || file.mimetype == "image/jpg" || file.mimetype == "image/jpeg"){
      cb(null, true);
    } else {
